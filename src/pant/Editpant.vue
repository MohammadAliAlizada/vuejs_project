<template>
  <div>
    <!-- <b-btn v-b-modal.modalPrevent size="sm" variant="primary" @click="myPant"><i class="plus icon" style="margin-left:5px;"></i>پتلون جدید</b-btn> -->
     <a  v-b-modal.modalPrevent class="btn edit ng-star-inserted" style="margin-left:4px;" @click="myPant" >
...
export default {
  ...
  methods: {
    myPant(){
        this.pant.customer_id = this.editpant.customer_id;
        this.pant.pant_length = this.editpant.pant_length;
        this.pant.waist = this.editpant.waist;
        this.pant.seat = this.editpant.seat;
        this.pant.knee = this.editpant.Knee;

        this.pant.cuff = this.editpant.cuff;
        this.pant.zep_ber_fly = this.editpant.zepberfly;
        this.pant.thigh = this.editpant.thigh;


        this.pant.orders = this.editpant.order;

        this.pant.total = this.editpant.total;

        this.pant.received = this.editpant.received;

      

        this.pant.minus = this.editpant.minus;

        this.pant.start_date = this.editpant.startDate;

        this.pant.end_date = this.editpant.endDate;


        if(this.editpant.isComplete == "تحت کار"){
           
            this.pant.iscomplete = 1;
           
        }else{
          
            this.pant.iscomplete = 2;
        }


        

        if(this.editpant.palet == "بدون پلیت"){

            this.pant.palet = 1;

        }else if(this.editpant.skirtDesign == "یک پلیت"){
    ...
    handleSubmit () {
       this.axios.defaults.headers.common["Authorization"] = 'Bearer ' + localStorage.getItem('token');
           this.axios.patch('/pants/'+ this.id , this.$data.pant)
           .then((response) => {
